...
function call_api() {
		...
		if (start > end){
			var text = "Start date must be less than end date!!"
			showNotificationError('danger', null, null, null, null, null, text)
		}
		
		$.ajax({
		url: "http://localhost:5000/search/date",
		method: 'POST',
...
$(document).on('click', "#search", function(e){
	...
	if (query == ""){
		...
		var ed = $("#end_date").val();
		if(sd!='' && ed!=''){
			localStorage.start_date = $("#start_date").val();
			localStorage.end_date = $("#end_date").val();
		}
...
$(document).on('click', "#datesearch", function(e){
	localStorage.page = 1
	call_api();
})
